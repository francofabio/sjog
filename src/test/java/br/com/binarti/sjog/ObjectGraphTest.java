...
public class ObjectGraphTest {
	...
	public void shouldCreateGraphObject() {
		...
		ObjectGraph johnGraph = new ObjectGraphBuilder().build(john);
	...
	public void shouldExcludePropertyFromGraphObject() {
		Person john = new Person("John Smith", 30);
		ObjectGraph johnGraph = new ObjectGraphBuilder()
				.exclude("age")
				.build(john);
	...
	public void shouldIncludeNestedPropertyFromObjectInCollectionInferCollectionType() {
		...
		ObjectGraph orderGraph = new ObjectGraphBuilder()
				.include("itens")
				.build(order);
	...
	public void shouldReturnNullIfNullReferenceIsUsedInObjectGraph() {
		ObjectGraph nullGraph = new ObjectGraphBuilder()
				.include("address")
				.build(null);
	...
	@Test
	public void shouldExcludeNestedPropertyInCollection() {
		Order order = new Order(129, new Date(), 1650d);
		order.addItem(new Item(1, "MacBook pro 13"));
		order.addItem(new Item(2, "iPad Air 2"));
		order.addItem(new Item(3, "iPad Mini"));
		ObjectGraph orderGraph = new ObjectGraphBuilder()
				.include("itens")
				.exclude("itens.id")
				.build(order);
		assertEquals(4, orderGraph.getRoot().getChildren().size());
		assertNotNull(orderGraph.getNode("id"));
		assertNotNull(orderGraph.getNode("date"));
		assertNotNull(orderGraph.getNode("amount"));
		assertNotNull(orderGraph.getNode("itens"));

		assertTrue(orderGraph.getNode("itens").isCollection());
		assertEquals(3, orderGraph.getNode("itens").getChildren().size());

		assertEquals(1, orderGraph.getNode("itens[0]").getChildren().size());
		assertNull(orderGraph.getNode("itens[0].id"));
		assertNotNull(orderGraph.getNode("itens[0].productName"));

		assertEquals(1, orderGraph.getNode("itens[1]").getChildren().size());
		assertNull(orderGraph.getNode("itens[1].id"));
		assertNotNull(orderGraph.getNode("itens[1].productName"));
	...
	public void shouldGetDataFromPolymorphismCollection() {
		...
		ObjectGraph docGraph = new ObjectGraphBuilder()
				.include("events")
